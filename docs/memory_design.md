...
这些文件会被程序截取为 `project_evidence` 后交给短 LLM；短 LLM 只有引用有效 `evidence_id` 时，才能创建或更新 `project_fact`。
...
触发后，摘要任务由单线程后台调度器执行，不阻塞用户当前问答。若上一轮摘要任务尚未完成，本轮不会重复提交新的摘要任务；`/reset` 会丢弃未完成摘要结果，避免旧任务在状态清空后写回。

这样设计是为了避免两个问题：

- CLI 初始化阶段产生不可见成本。
- 启动后没有用户行为，却发生不可预期写入。
- 短时间连续问答时，后台摘要不会堆积成多任务并发写入。
