...
配置必须存在于启动 CLI 的进程环境中。AutoPatch-J 不会自动读取 `.env`。其中 `AUTOPATCH_LLM_API_KEY` 必须设置，接口地址和模型有默认值。
...
| `AUTOPATCH_LLM_MODEL` | `deepseek-v4-flash` | 供应商提供的模型名 |
| `AUTOPATCH_LLM_REASONING_EFFORT` | 空 | 可选值由模型供应商决定 |
...
Memory 可能包含未脱敏的用户原文和模型服务诊断。查看或分享导出文件前，应先检查其中是否有敏感信息。
...
- `403`：检查模型权限、账户余额和供应商访问策略。
- `404`：检查 `AUTOPATCH_LLM_BASE_URL` 和 `AUTOPATCH_LLM_MODEL`。

需要查看模型服务返回的原始错误时，设置 `AUTOPATCH_DEBUG=true` 并重启 CLI。错误正文可能包含敏感信息，分享前需要检查。
