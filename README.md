...
| Principle | 说明 |
|---|---|
| **workflow owns state, agent owns reasoning** | Workflow 负责意图、scope、扫描、finding 队列、补丁队列和人工确认；Agent 负责解释问题、读取证据、调用工具、生成或重写补丁草案，避免把系统状态交给 LLM 自行维护。 |
| **scanner provides evidence, llm performs triage** | Semgrep 提供可定位 finding；LLM 基于 finding、源码片段和当前 scope 做取证、解释和最小修复。PMD、SpotBugs、Checkstyle 当前作为 planned scanner 展示。 |
| **function tools are gated by intent** | 不同 `IntentType` 对应不同工具白名单，让模型能使用工具，但不能把一个聊天问题扩展成任意文件读取或补丁修改。 |
| **patch is a review item, not a chat reply** | 补丁进入人工确认队列，包含目标文件、关联 finding、unified diff、修复理由和校验状态；默认只重写当前补丁，不自动改动后续队列。 |
| **memory helps chat, not repair** | 项目级 memory 只服务 `code_explain` 和 `general_chat`，不进入 `code_audit`、`patch_explain`、`patch_revise`。详细设计见 [Agent Memory 设计说明](docs/memory_design.md)。 |
| **llm calls are purpose driven** | 业务只声明 `LLMCallPurpose`；主 Agent 推理支持流式输出，意图识别和记忆摘要关闭 reasoning，底层供应商参数由 LLM 层统一处理。 |
...
└─ core/      # 领域模型、意图、范围、扫描、review、patching、memory、项目索引
