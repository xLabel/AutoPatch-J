...
def test_chat_filter_service_limits_general_chat_to_programming_topics() -> None:
...
def test_chat_filter_service_compacts_long_markdown_answer_by_default() -> None:
    service = ChatFilterService()
    answer = (
        "## 常见解法\n"
        "1. 暴力枚举\n"
...
def test_chat_filter_service_keeps_more_detail_when_user_explicitly_requests_code() -> None:
    service = ChatFilterService()
    answer = (
        "## Python 示例\n"
        "```python\n"
...
def test_chat_filter_service_detects_explicit_detail_and_code_requests() -> None:
    service = ChatFilterService()
...
def test_chat_filter_service_out_of_scope_reply_has_no_garbled_text() -> None:
    service = ChatFilterService()
