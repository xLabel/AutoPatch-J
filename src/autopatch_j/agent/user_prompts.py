...
def build_code_audit_user_prompt(
    ...
    sections: list[PromptSection] = [
        PromptSection(
            ...
            f"- finding: {current_finding.finding_id}\n"
    ...
    if force_reread:
        sections.append(
            PromptSection(
                ...
                f"这一次你必须先用 read_source_context(path={current_finding.file_path}, line={current_finding.start_line}) "
                "或 read_source_block 重新确认源码，再重新 propose_patch。",
            )
        )
    sections.append(PromptSection("用户原始请求", text))
...
def build_code_explain_user_prompt(
    ...
    if scope.kind is CodeScopeKind.SINGLE_FILE:
        return render_prompt_sections(
            PromptSection("解释范围", f"- 文件: {scope.focus_files[0]}"),
            PromptSection(
                "回答要求",
                "- 只基于当前文件可见内容解释代码功能。\n"
                "- 不要主动搜索、读取或推断焦点范围外的类型实现、调用方或配置来源。\n"
                "- 如果出现外部类型名，只能基于当前文件里的使用方式做保守说明。\n"
                "- 回答默认控制在 2 到 4 句；除非用户明确要求详细展开，否则不要输出分节报告。",
            ),
            PromptSection("用户问题", text),
        )
    ...
    if scope.kind is CodeScopeKind.PROJECT:
        ...
        sections: list[PromptSection] = [
            PromptSection("当前任务", "当前任务是项目级代码讲解。不要触发扫描，不要提出补丁。"),
            PromptSection(
                "回答要求",
                "- 基于项目结构、索引和少量必要源码证据回答。\n"
                "- 默认控制在 3 到 6 行，优先说明项目用途推断、主要结构、关键类线索和判断依据。\n"
                "- 如果信息不足，必须说明只能根据当前文件结构和可见源码推断。\n"
                "- 最近扫描 ID（如 scan-xxxx）只是扫描记录，不是项目名称或项目描述。",
            ),
        ]
        if project_context:
            sections.append(PromptSection("项目上下文", project_context))
        sections.extend(
            [
                PromptSection("项目 Java 文件清单", listed_files or "- 无 Java 文件"),
                PromptSection("用户问题", text),
            ]
        )
        return render_prompt_sections(*sections)
    ...
    return render_prompt_sections(
        PromptSection("当前任务", "当前任务是代码讲解。"),
        PromptSection("当前焦点范围", joined_paths),
        PromptSection(
            "回答要求",
            "- 可以在当前焦点范围内使用 search_symbols 和源码读取工具辅助解释。\n"
            "- 不要越过当前 focus scope。\n"
            "- 回答默认控制在 1 段或 3 个要点以内；除非用户明确要求详细展开，否则不要输出长篇报告。",
        ),
        PromptSection("用户问题", text),
    )
...
def build_patch_explain_user_prompt(current_item: ReviewPatchItem, user_text: str) -> str:
    ...
    return render_prompt_sections(
        PromptSection(
            "当前待确认补丁",
            f"- 文件: {current_item.file_path}\n"
            f"- 补丁意图: {draft.rationale or '无说明'}",
        ),
        PromptSection("补丁差异", f"```diff\n{draft.diff}\n```"),
        PromptSection(
            "回答要求",
            "1. 先直接回答用户问题。\n"
...
def build_patch_revise_user_prompt(
    ...
    return render_prompt_sections(
        PromptSection(
            "当前待重写补丁",
            f"- 文件: {current_item.file_path}\n"
            f"- 关联 finding: {finding_handles}\n"
            f"- 补丁意图: {draft.rationale or '无说明'}",
        ),
        PromptSection("当前补丁差异", f"```diff\n{draft.diff}\n```"),
        PromptSection("用户反馈", user_text),
        PromptSection(
            "执行要求",
            "- 只重写当前补丁，不要修改、删除或重建后续补丁。\n"
